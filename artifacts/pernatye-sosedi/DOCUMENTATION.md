...
Поля «Даты отъезда» в обоих экранах (`/new-request` и `/edit-request/[id]`) — это нативный календарь `@react-native-community/datetimepicker`. По тапу на кнопку с иконкой 📅 открывается системный date picker (на Android в режиме `display="default"`, на iOS — спиннер). Состояние хранится как `Date`, в Postgres всегда уходит ISO-строка `YYYY-MM-DD` через `toISODate(date)`. Ограничения: у даты «С» — `minimumDate={new Date()}` (нельзя выбрать прошлое), у даты «По» — `minimumDate={dateFrom}` (нельзя выбрать раньше начала); если пользователь сдвинул «С» позже «По», «По» автоматически подтягивается к «С». Локаль пикера — `ru-RU` (форматирование месяцев и кнопок), русская локаль также объявлена в `app.json → expo.locales.ru = ./locales/ru.json` (`{ "CFBundleLocalizations": ["ru"] }`), что нужно App Store для отображения языка интерфейса. Дополнительная серверная нормализация формата сохраняется в `pernatye-sosedi-api` (`POST /api/sit-requests` → `convertDate`) — на случай старых клиентов с `DD.MM.YYYY`.
